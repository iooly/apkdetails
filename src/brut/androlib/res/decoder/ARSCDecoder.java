...
public class ARSCDecoder {
    ...
    private ResPackage[] readTable() throws IOException, AndrolibException {
        ...
        for (int i = 0; i < packageCount; i++) {
            ...
            // System.out.println("res pkg: [" + i + "] " + packages[i]);
        }
    ...
    private ResBagValue readComplexEntry() throws IOException,
        ...
        ArrayList<Duo<Integer, ResScalarValue>> duoList = new ArrayList<Duo<Integer, ResScalarValue>>();
        int x;
        ResValue value = null;
        for (int i = 0; i < count; i++) {
            x = mIn.readInt();
            value = readValue();
            if (value instanceof ResScalarValue) {
                duoList.add(new Duo<Integer, ResScalarValue>(x,
                        (ResScalarValue) value));
            }
        }
        int size = duoList.size();
        Duo<Integer, ResScalarValue>[] items = new Duo[size];
        for (int i = 0; i < size; i++) {
            items[i] = duoList.get(i);
        }
